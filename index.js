...
const { findMovies, findMoviesByLocation, findMovies_Abdal } = require('./SearchMovies');
...
async function savePFP(UID, base64EncodedImage, client, res) {
  try{
    const query = { UID: UID }
    console.log('UID:', UID);
    const updateResult = await client.db("abdal-test").collection("test").updateOne(query, { $set: { hasPFP: true } });
    // console.log('base64', base64EncodedImage)
    const pfp = {
      UID,
      base64EncodedImage,
    };
    const result = await client.db("abdal-test").collection("PFP").findOne(query);
    if (result) {
      const result1 = await client.db("abdal-test").collection("PFP").updateOne(query, { $set: { base64EncodedImage: base64EncodedImage } });
    }
    else
    {
      const result1 = await client.db("abdal-test").collection("PFP").insertOne(pfp);
    }
  } catch (error) {
...
async function getPFP(UID, client, res) {
  try{
    const query = { UID: UID }
    console.log('UID:', UID);
    const updateResult = await client.db("abdal-test").collection("test").findOne(query);
...
async function searchTitle(titleString, client, res) {
  try {
    ...
    const result = await client.db("Location-db")
      ...
      .aggregate([
        ...
        { $group: { _id: { title: "$TITLE", year: "$YEAR" }, docs: { $push: "$$ROOT" } } } // Group by TITLE and YEAR and push all documents into an array
    ...
    if (result) {
      const simplifiedresult = result.reduce((accumulator, group) => {
        ...
        const hasValidCoordinates = group.docs.some(doc => !isNaN(doc.LATITUDE) && !isNaN(doc.LONGITUDE));
        if (hasValidCoordinates) {
          // Include the title in the result if any document has valid coordinates
          accumulator.push({ _id: group.docs[0]._id.toString(), title: group._id.title, year: group._id.year });
        }
        return accumulator;
      }, []);
...
async function searchLocation(locationString, client, res) {
  try {
    ...
    const result = await client.db("Location-db")
      .collection("Locations")
      .aggregate([
        { $match: { LOCATION: { $regex: `^${locationString}`, $options: 'i' } } }, // Match titles containing "iron"
        { $sort: { _id: 1 } }, // Sort by _id to ensure consistent first occurrences
    ...
    if (result) {
      ...
      const simplifiedresult = result
        ...
        .map(({ _id, LOCATION, ADDRESS }) => ({ _id: _id.toString(), location: LOCATION, address: ADDRESS }));
...
async function sendUID(UID, username, firstName, lastName, email, lastRequestTimestamp, hasPFP, client, res) {
  try {
    console.log(UID);
    ...
    //took off insertMany
    const result = await client.db("abdal-test").collection("test").insertOne(newUser);
...
async function EditProfile(UID, FirstName, LastName, client, res) {
  try {
    ...
    // console.log(LastName);
    const query = { UID };
    const result = await client.db("abdal-test").collection("test").updateOne(
      query,
      {
...
async function searchUser(username, client, res) {
  try {
    ...
    // const result = await client.db("abdal-test").collection("test").find({ user: { $regex: `${user}`, $options: 'i' } }).toArray();
    const result = await client.db("abdal-test").collection("test").find({ username: { $regex: `${username}`, $options: 'i' } }).toArray();
    console.log(result);
    if (result) {
      const usernames = result.map(({ _id, UID, username, firstName, lastName, hasPFP }) => ({ _id: _id, UID: UID, username: username, firstName: firstName, lastName: lastName, hasPFP: hasPFP }));
...
async function fetchFollowers(UID, client, res) {
  try {
    console.log('UID: ', UID);
    // const result = await client.db("abdal-test").collection("test").find({ user: { $regex: `${user}`, $options: 'i' } }).toArray();
    const result = await client.db("abdal-test").collection("follow").find({ followingUID: { $regex: `${UID}`, $options: 'i' } }).toArray();
    console.log("result in backend followers: " + result);
    if (result) {
      const followers = result.map(({ followerUID }) => ({ followerUID: followerUID }));
...
async function fetchFollowing(UID, client, res) {
  try {
    console.log('UID: ', UID);
    // const result = await client.db("abdal-test").collection("test").find({ user: { $regex: `${user}`, $options: 'i' } }).toArray();
    const result = await client.db("abdal-test").collection("follow").find({ followerUID: { $regex: `${UID}`, $options: 'i' } }).toArray();
    console.log("result in backend following: " + result);
    if (result) {
      const following = result.map(({followingUID}) => ({followingUID: followingUID}));
...
async function searchUsername(username, client, res) { //function to search username and see if its valid or not, used during signup
  try {
    console.log(username);
    const result = await client.db("abdal-test").collection("test").findOne({ username: { $regex: `^${username}$`, $options: 'i' } });
    if (result === null) {
...
async function followUser(followerUID, followingUID, client, res) {
  try {
    ...
    //took off insertMany
    const result = await client.db("abdal-test").collection("follow").insertOne(newFollow);
    // console.log(`${result.insertedCount} new UID was inserted with the following id:`);
    // console.log(result.insertedIds);
    // Send the result back to the HTTP client
    res.json({ type: 'sendUID', data: 'UID was inserted successfully' });
...
async function unfollowUser(followerUID, followingUID, client, res) {
  try {
    ...
    //took off insertMany
    const result = await client.db("abdal-test").collection("follow").deleteOne(newUnfollow);
    // console.log(`${result.insertedCount} new UID was inserted with the following id:`);
    // console.log(result.insertedIds);
    // Send the result back to the HTTP client
    res.json({ type: 'sendUID', data: 'UID was inserted successfully' });
...
async function fetchRequestTimestamp(UID, client, res) { //function for email timestamp
  try {
    ...
    if (timestamp === "") {
      ...
      //insert current date
      const result1 = await client.db('abdal-test').collection('test').updateOne(
        query,
        {
      ...
      if (timeDifferenceMs >= duration60SecondsMs) { //if 60 sec or more have passed
        ...
        res.json({ type: 'fetchRequestTimestamp', data: true, timestamp, currentDate});
      } else { //less than 60 seconds have passed
        // Do something else if the time difference is less than 60 seconds
        console.log("The time difference is less than 60 sec.");
        res.json({ type: 'fetchRequestTimestamp', data: false, timestamp, currentDate});
      }
    }
  } catch (error) {
...
async function getLocationsFromCoordinates(result, client, res) {
  try {
    ...
    for (const location of result) {
      ...
      const result = await client.db('Location-db').collection('Locations').findOne(query);
      array1.push(result);
      // const result = await cursor.toArray();
...
async function getScoutedCoordinates(UID, client, res) {
  try {
    const query = { UID };
    const cursor = await client.db('abdal-test').collection('scoutedLocations').find(query);
    const result = await cursor.toArray();
    ...
    for (const location of result) {
      ...
      if (location.hasOwnProperty('coordinatesOfLocation')) {
        ...
        console.warn('Warning: "coordinatesOfLocation" property not found in an element.');
      }
    }
...
async function sendScoutedLocation(UID, titleScouted, coordinatesOfLocation, client, res) {
  try {
    const query = {
      UID,
      titleScouted,
    ...
    const result = await client.db("abdal-test").collection("scoutedLocations").insertOne(query);
    if (result) {
...
async function UndoFetchRequestTimestamp(UID, timestamp, client, res) { //function for email timestamp
  try {
...
async function findLocation(location, client, res) {
  try {
    const locationWithMessage = await findMoviesByLocation(client, { Location: location });
...
async function doGetTitlesByLocation(location, client, res) {
  const queryResults = await getTitlesByLocation(client, { Location: location });
  ...
  try {
    ...
    if (connectionStatus) {
      ...
      app.post('/sendUID', async (req, res) => {
        const { UID, username, firstName, lastName, email, lastRequestTimestamp, hasPFP} = req.body;
        ...
        await sendUID(UID, username, firstName, lastName, email, lastRequestTimestamp, hasPFP, client, res);
      });
      ...
      app.post('/sendScoutedLocation', async (req, res) => {
        ...
        // console.log('erm');
        // console.log('Received UID:', UID);
        await sendScoutedLocation(UID, titleScouted, coordinatesOfLocation, client, res);
      });
      ...
      app.post('/fetchRequestTimestamp', async (req, res) => { //for email timestamp
        const { UID } = req.body;
        console.log('Received UID:', UID); // Log the UID
        await fetchRequestTimestamp(UID, client, res);
      });
      app.post('/UndoFetchRequestTimestamp', async (req, res) => { //for email timestamp
...
async function findMovieTitle(client, {
  Title = null,
  resultsLimit = Number.MAX_SAFE_INTEGER
} = {}) {
  const cursor = client.db("Location-db").collection("Locations").find({ TITLE: Title }).limit(resultsLimit);
  const results = await cursor.toArray();
  return results;
}

//Searches database for location with more than one qualifier (i.e. LOCATION, CITY, COUNTRY, etc.)
async function findMovieLocation(client, {
  Location = null,
  resultsLimit = Number.MAX_SAFE_INTEGER
} = {}) {
...
app.get('/fetch-movie/:movieTitle', (req, res) => {
  let resList = findMovieTitle(dbClient, { Title: req.params.movieTitle })
  resList.then((data) => {
    res.status(200).send(data);
  });
});
...
app.get('/fetch-location/:movieLocation', (req, res) => {
  let resList = findMovieLocation(dbClient, { Location: req.params.movieLocation })
  resList.then((data) => {
    res.status(200).send(data);
  })
})
